...
public class Infernor implements Listener
{
    ...
    public void aoComando(final PlayerCommandPreprocessEvent e) {
        ...
        if (Infernor.lutando.containsKey(p.getName()) && e.getMessage().startsWith("/")) {
            e.setCancelled(true);
            p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea n\u00e3o pode utilizar comando quanto estiver lutando");
        }
    }
        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.NETHER_FENCE && Habilidade.getAbility(p).equalsIgnoreCase("Infernor")) {
                ...
                if (this.generateGLASS) {
                    ...
                    p.sendMessage(String.valueOf(API.preffix) + "§7Voce desafiou um jogador! Voce tem 5 segundos de invencibilidade!");
                    p.sendMessage(String.valueOf(API.preffix) + "§7Caso nao tenha nenhum vencedor depois de 4 minutos, voce voltara ao seu local de origem!");
                    r.sendMessage(String.valueOf(API.preffix) + "§7Voce foi desafiado! Voce tem 5 segundos de invencibilidade!");
                    r.sendMessage(String.valueOf(API.preffix) + "§7 Caso nao tenha nenhum vencedor depois de 4 minutos, voce voltara ao seu local de origem!");
                    ...
                    this.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.getInstance(), (Runnable)new Runnable() {
                        ...
                        public void run() {
                            if (Infernor.lutando.containsKey(p.getName()) && Infernor.lutando.get(p.getName()).equalsIgnoreCase(r.getName()) && Infernor.lutando.containsKey(r.getName()) && Infernor.lutando.get(r.getName()).equalsIgnoreCase(p.getName())) {
                                ...
                                p.sendMessage(String.valueOf(API.preffix) + "§7N\u00e3o houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
                                r.sendMessage(String.valueOf(API.preffix) + "§7N\u00e3o houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
                                r.setFireTicks(0);
                                p.setFireTicks(0);
