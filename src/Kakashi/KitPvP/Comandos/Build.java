            ...
            if (p.hasPermission("kitpvp.build")) {
                if (args.length == 0) {
                    if (!Build.embuild.contains(p)) {
                        Build.embuild.add(p);
                        p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea ativou seu modo build §2✔");
                    }
                    else {
                        Build.embuild.remove(p);
                        p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea desativou seu modo build §4✗");
                    }
                }
                else {
                    ...
                    if (!Build.embuild.contains(t)) {
                        Build.embuild.add(t);
                        p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea ativou o modo build de: §7" + t.getName());
                    }
                    else {
                        Build.embuild.remove(t);
                        p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea desativou o modo build de: §7" + t.getName());
                    }
                }
            }
            else {
