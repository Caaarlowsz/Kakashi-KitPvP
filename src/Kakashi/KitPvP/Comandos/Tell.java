...
public class Tell implements CommandExecutor
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (label.equalsIgnoreCase("tell")) {
            if (args.length == 0) {
                p.sendMessage(String.valueOf(API.preffix) + "§cEscreva: /tell (nome) (msg) §2✔");
                return true;
            }
            final Player target = Bukkit.getPlayer(args[0]);
            if (args[0].toLowerCase().equalsIgnoreCase("on")) {
                Tell.telloff.remove(p);
                p.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea §bativou §7seu tell §2✔");
            }
            else if (args[0].toLowerCase().equalsIgnoreCase("off")) {
                Tell.telloff.add(p);
                p.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea §bdesativou §7seu tell §4✗");
            }
            if (args.length > 1) {
                if (target == null) {
                ...
                if (Tell.telloff.contains(target)) {
                    p.sendMessage(String.valueOf(API.preffix) + "§cO tell desse player esta off §4✗");
                    return true;
                }
            ...
            if (args.length == 0) {
                p.sendMessage(String.valueOf(API.preffix) + "§7fale com o player no chat normal §2✔");
                return true;
            }
            if (!Tell.gettell.containsKey(p)) {
                p.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea n\u00e3o tem ninguem para responder §4✗");
                return true;
            }
