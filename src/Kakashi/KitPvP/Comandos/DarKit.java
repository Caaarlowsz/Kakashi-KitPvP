    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("darkit")) {
            if (p.hasPermission("kitpvp.darkit")) {
                if (args.length == 0) {
                    p.sendMessage(String.valueOf(API.preffix) + "§7Escreva: /darkit <player> <kit> §2✔");
                    return true;
                }
                if (args.length == 1) {
                    p.sendMessage(String.valueOf(API.preffix) + "§7Escreva: /darkit <player> <kit> §2✔");
                ...
                if (m.length() >= 16) {
                    p.sendMessage(String.valueOf(API.preffix) + "§7Esse kit n\u00e3o existe §4✗");
                    return true;
                }
                if (m.length() <= 1) {
                    p.sendMessage(String.valueOf(API.preffix) + "§7Esse kit n\u00e3o existe §4✗");
                ...
                p.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea deu o kit: §5" + m + " §7para: §4" + t.getName());
            }
            else {
