...
import Kakashi.KitPvP.Sistemas.TagsAPI;
		...
		if (args.length != 0) {
			if (args[0].equalsIgnoreCase("dono")) {
				if (p.hasPermission("tag.dono")) {
					...
					p.sendMessage(String.valueOf(API.preffix) + "§4Tag §7 alterada");
					return true;
				}
			...
			if (args[0].equalsIgnoreCase("coder")) {
				if (p.getName().equalsIgnoreCase("IZ44KPvP_")) {
					...
					p.sendMessage(String.valueOf(API.preffix) + "§4Tag §7 alterada -> IZ44KPvP_ :D");
					return true;
				}
