...
public class Congelar implements CommandExecutor, Listener
{
            ...
            if (p.hasPermission("kitpvp.staff")) {
                if (args.length == 0) {
                    ...
                    if (!Congelar.congelado.contains(t)) {
                        ...
                        t.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea foi congelado por: §c" + p.getDisplayName());
                        ...
                        t.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea foi descongelado por: §a" + p.getDisplayName());
                    ...
                    if (Congelar.congelado.contains(t)) {
                        Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, (Runnable)new Runnable() {
                            ...
                            public void run() {
                                Congelar.congelado.remove(t);
                                t.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea foi descongelado depois de uma hora");
                            }
                        }, 72000L);
                    }
                }
    ...
    public void aoComando(final PlayerCommandPreprocessEvent e) {
        ...
        if (Congelar.congelado.contains(p) && e.getMessage().startsWith("/")) {
            e.setCancelled(true);
            p.sendMessage(String.valueOf(API.preffix) + "§cVoc\u00ea n\u00e3o pode digitar comando quando estiver congelado");
        }
    }
