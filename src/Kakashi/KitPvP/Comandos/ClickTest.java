...
public class ClickTest implements CommandExecutor, Listener
{
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (cmd.getName().equalsIgnoreCase("clicktest")) {
            ...
            Bukkit.getScheduler().scheduleSyncDelayedTask(Main.plugin, (Runnable)new Runnable() {
                ...
                public void run() {
                    if (p.getLevel() >= 200) {
                        Bukkit.broadcast("§4§l" + p.getDisplayName() + " §7Pode estar de macro quantidade de clicks no clicktest: §4§l" + p.getLevel(), "flame.staff");
    ...
    public void comandos(final PlayerCommandPreprocessEvent e) {
        ...
        if (ClickTest.emclicktest.contains(p.getName()) && e.getMessage().startsWith("/")) {
            e.setCancelled(true);
            p.sendMessage(String.valueOf(API.preffix) + "§7Voc\u00ea n\u00e3o pode digitar comandos quando estiver fazendo clicktest §4✗");
        }
    }
